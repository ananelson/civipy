...
class CiviContribution(CiviCRMBase):
    ...
    def find_by_transaction_id(cls, trxn_id: str):
        ...
        return cls.find(trxn_id=trxn_id)

    @classmethod
    def find_by_invoice_id(cls, invoice_id: str):
        return cls.find(invoice_id=invoice_id)

    @classmethod
    def find_by_donor(cls, display_name: str, total_amount: float | None = None, receive_date: str | None = None):
        """Find a contribution by donor's display name, and optionally
        by amount and/or date received (yyyy-mm-dd)."""
        contact = CiviContact.find(display_name=display_name)
        return cls.find_by_donor_id(contact["contact_id"], total_amount, receive_date)

    @classmethod
    def find_by_donor_id(cls, contact_id: int, total_amount: float | None = None, receive_date: str | None = None):
