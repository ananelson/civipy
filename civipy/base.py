...
class CiviCRMBase(object):
    ...
    @classmethod
    def process_http_response(cls, r: "BaseHTTPResponse"):
        logger.debug(r.url)
        if r.status == 200:
    ...
    def http_get(cls, action, entity, kwargs):
        ...
        r = urllib3.request("GET", SETTINGS.rest_base, fields=params)
    ...
    def http_post(cls, action, entity, kwargs):
        params = cls._params(entity, action, kwargs)
        logger.debug(str(params))
        r = urllib3.request("POST", SETTINGS.rest_base, fields=params)
        return cls.process_http_response(r)
    ...
    def run_drush_process(cls, action, entity, params):
        ...
        p = subprocess.Popen(
            [SETTINGS.rest_base, "cvapi", "--out=json", "--in=json", "%s.%s" % (entity, action)],
    ...
    def run_cvcli_process(cls, action, entity, params):
        ...
        p = subprocess.Popen(
            [SETTINGS.rest_base, "-e", entity, "-a", action, "--json"] + params, stdout=subprocess.PIPE
        )
