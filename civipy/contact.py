...
class CiviRelationship(CiviCRMBase):
    ...
    def create_or_increment_relationship(
        cls, contact_id_a, contact_id_b, relationship_type_id, event_id=None, activity_id=None
    ):
        print(
            "in create_or_increment_relationship with a %s b %s type %s"
...
class CiviGroup(CiviCRMBase):
    ...
    def add_member(self, civi_contact):
        CiviGroupContact.find_or_create(
            search_key_name=["contact_id", "group_id"], contact_id=civi_contact.civi["id"], group_id=self.civi["id"]
        )
